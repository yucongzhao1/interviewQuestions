...
## **CDN 分发（减少传输时间）：
**通过多台服务器部署相同的副本，当用户访问的时，服务器根据用户跟哪台服务器距离距离近，来决定哪台服务器去响应这个请求。

## **后端在业务层的缓存：
**数据库查询缓存是可以设置缓存的，这个对于处于高频率的请求很有用。浏览器一般不会对于 `content-type:application/json` 的接口进行缓存；所以有时需要我们手动地为接口设置缓存。比如一个用户的签到状态，它的缓存时间可以设置到明天之前。
...
##  **减少请求的数量**：
这点在 http1.1的优势很明显，因为 http1.1 的请求是串行的（尽管多个 tcp 通道），每个请求都需要往返后才能请求下一个请求。此时合并请求可以减少在路途上浪费的时间，此外还会带来重复的请求头部信息（比如 cookie）。在 http2.0 中这个问题会弱化很多，但也有做的必要。

## **页面使用骨架屏：
**意思是在首屏加载完成之前，通过渲染一些简单的元素占位。骨架屏的好处在于可以减少用户等待时的急躁情绪。这点很有效，在很多成熟的（京东丶淘宝丶YouTube）都有大量应用。没有骨架屏的话，一个 loading 的菊花图也是可以的。

- **使用 SSR 渲染**

## **引入 HTTP2.0：
**HTTP2.0 对比 HTTP1.1，最重要的提升是传输性能，在接口小而多的时候更加明显。

## **利用好 HTTP压缩：
**即使是最普通的 gzip，也能把 boostrap.mini.css 压缩到原来的17%。可见，压缩的效果非常明显，特别是对于文本类的静态资源。另外，接口也是能压缩的。接口不大的话不用压缩，因为性价比低（考虑压缩和解压的时间）。

## **利用好 script 标签的 async 和 defere 这两个属性：
**功能独立且不要求马上执行的 JS 文件，可以加上 async 属性。如果是优先级低且没有依赖的 JS，可以加上 defere 属性。

## **（少用）选择先进的图片格式：
**使用 WebP 的图片格式来代替现有的 jpeg 和 peg，当页面图片较多时，这点作用非常明显。把部分大容量的图片从 B

##  **目前（少用）渲染的优先级：
**浏览器有一套资源的加载优先级策略，也可以通过 JS 来自己控制请求的顺序和渲染的顺序。一般我们不需要这么细粒度的控制，而且控制的代码也不好写。
...
- CommonJS 模块运行时加载， ES6 模块是编译时输出接口
  ...
  - 编译时加载：ES6 模块不是对象。而是通过 export 命令显示指定输出的代码，(import 时采用静态命令的形式。即)在 import 时可以指定加载某个输出值，而不是加载整个模块，这种加载称为”编译时加载“。
...
## 18-1. 浅拷贝
浅拷贝是对对象地址的复制, 拷贝的结果是两个对象指向同一个地址
...
newArr[0].a = 123;
console.log(arr[0]);
// 但是这个方法会存在 引用失效 的问题var b = {};var a = {a1: b, a2: b};console.log(a.a1 === a.a2);var c = deepClone(a);console.log(c.a1 === c.c2);
```
...
// 测试var arr = [2, 1, 3, 4, 1, 1, {}, {}];console.log(unique(arr));
```
...
```js
var arr = [1, 2, 3, 4, 5];
var newArr = arr.sort((a, b) => (Math.random() > 0.5 ? -1 : 1));
console.log(newArr);
```
...
function shuffle(arr) {
    ...
    for (let i = len - 1; i >= 0; i--) {
      randomIndex = Math.floor(Math.random() * (i + 1)); // 交换位置
...
```js
function ajax(method, url, data, success, boolean) {
    var xhr = null;
...
- postMessage 跨域
- 跨域资源共享（CORS） Access-Control-Allow-original
- Nginx 代理
- NodeJs 中间件代理跨域
- WebSocket 协议跨域
...
#### CORS请求的过程（Cross-Origin Resource Sharing）
1. 浏览器：发现为跨源请求，直接发出CORS请求，自动在头信息中，添加一个Origin字段

2. 服务器：根据Origin字段中的信息：
...
# 27. 前端存储的方式 cookie session localStorage
...
# 30. 数据类型判断
  ## typeof
    基本类型除了null为object， 其他为基本类型
    对选哪个类型除了函数为function外，其他为object
  
  ## instanceof 
    用来判断对象数据类型的具体类型： Object，Array
    对象  instanceOf  构造函数
...
能检测基本数据类型
  ...
  in 运算符只能检查某个属性或方法是否可以被对象访问，不能检查是否是自己的属性和方法 
...
区别：
 ...
 4. for in会遍历对象的整个原型链，性能差；而for-of只遍历当前对象，不会遍历原型链

# 34. map和forEach的区别
  map() 会分配内存空间存储新数组并返回，forEach() 不会返回数据。
  forEach() 允许callback更改原始数组的元素。map() 返回新的数组。
  - 场景  
    我建议你在创建一个新的数组时使用map，当你不需要制作一个新的数组，而是要对数据做一些事情时，就使用forEach。

主要区别在于是否有返回值，所以你会希望使用 map 来制作一个新的数组，而使用 forEach 只是为了映射到数组上。

# 35. localStorage 和 SessionStorage
- 相同点
...
# 36. session 和 cookie的区别，操作cookie
## 1. cookie
...
### Create Cookies
  - 设置 Cookie
    ...
    开发者工具的 Application（Safari 或Chrome）或 Storage（Firefox）中找到该 cookie。
...
### Update Cookies
  - 可以通过创建的方式用新值覆盖 cookie 来更改它的值

### Delete Cookies
  - 可以给 cookie 设置一个空值，并将其过期日期设置为过去的任意时间来删除 cookie。
    ```js
...
3、Object.getPrototypeOf(p)  //推荐，规范写法
...
# 38. 如何自定义迭代器， 什么是迭代器， 可迭代的对象有

- 迭代器：对象的next方法是一个无参函数，它返回一个对象，该对象拥有done和value两个属性：
...
## 哪些操作是基于可迭代的
  ...
  promise.all(iterable)和promist.race(iterable)

https://juejin.cn/post/6873457657018728456#heading-9


# 39. reduce的源码实现和使用场景
 - https://www.jianshu.com/p/0d590e0f3afa
源码
```js
//方式一
...
## 使用场景
 ...
 - 计数器
  ...
  - 生成斐波那契数列
    ...
    ```
  - 删除指定位置的元素
    首先根据filter函数过滤出数组中符合条件的值，然后使用reduce在原数组中删除符合条件的值，
...
# 40. 包装类
  js包装类有三种： String(), Number(), Boolean();
  方法和属性只能添加给对象，不能添加给基本类型的数据，当我们对一些基本类型的数据类型的值去调用属性和方法时， 浏览器会临时使用包装类将其转化为对象，然后在调用对象的属性和方法，调用完以后，将其转化为基本类型。

# 41. 操作Dom的方法
- document.documentElement    获取html根元素
- document.all   获取所有的元素
- document.body  获取body
- document.getElementByClassName   根据属性名获取
- document.querySelector  根据css选择器查询一个元素（返回匹配的第一个）
- document.querySelectorAll   根据css选择器查询一组元素
- document.createElement  根据标签名创建元素
- document.createTExtNode  根据文本内容创建一个文本节点
- 父节点.appendChild 向父元素中添加子元素
- 父节点.insertBefore(新， 旧)   在新节点插入到旧节点前
- 父节点.replaceChild(新， 旧)
- 父节点.removeChild(zi)  === 子节点.parentNode.removeChild(子)

# 41. ESM的原理

# 44. some，every， filter和 map的
...
# 45. promise 和 Observable的区别

## 发送值
- Observable一个时间段内能发出多个值
- Promise只发一次 且只发一个值
...
## 懒加载
- Observable是懒加载的， observable只有被subscribe后才会被调用
- Promise 没有懒加载：.then 或者.catch后立即发送请求

## 取消
- Observable用unsubscribe可以被取消
- Promise不能取消

  Promise 的执行默认是不可取消的，而 Observable 是可以通过 subscribe 的 unsubscribe()方法来取消
因为在 Promise 中.then()返回的是一个新的 Promise 对象，而 Observable+subscribe 返回的是 subscription(订阅) 对象


## 操作符
- Observable提供map、foreach、retry, retrywhen、filter、reduce等操作符	
- Promise不提供操作符

# 46. set 和map的区别，map和普通对象的区别
## set 和map的区别
...
## 点( . )或者方括号( [ ] ) + undefined判断
- 点或者方括号可以获取对象的属性值，如果对象上不存在该属性，则会返回 undefined。这种方式可以判断指定对象的自有属性和继承属性
...
## in运算符
- 指定对象的自有属性和继承属性中是否存在某属性，如果存在则返回 true。
...
# 48. setInterval 缺点 与 setTimeout 的不同
...
## 为什么要用虚拟DOM：
 DOM操作很慢，轻微的操作都可能导致⻚面重新排 版，⾮常耗性能。相对于DOM对象，js对象处理起来更快， 而且更简单。通过diff算法对比新旧vdom之间的差异，可以 批量的、最⼩化的执行dom操作，从而提高性能
...
console.log("2--", Math.max.call(Math, arr[0], arr[1], arr[2],arr[3]));

console.log("4--", Math.max.apply(Math,arr))
...
- 面向对象的特性：
  ...
  封装性： 把客观事物封装成抽象的类，隐藏属性和方法的实现细节，仅对外公开接口
  继承性：子类可以使用父类的所有功能，并且对这些功能进行扩展。继承的过程，就是从一般到特殊的过程。
  [多态性]抽象： 同一操作作用于不同的对象上，可以产生不同的解释和不同的执行结果
    使用场景： 多态在设计模式中应用得比较广泛，比如 组合模式 / 策略模式等等。
...
## 概念
  ...
  主要的编程范式有三种：1.命令式编程，2.声明式编程和3.函数式编程

  相比命令式编程，函数式编程更加强调程序执行的结果而非执行的过程，

## 函数编程式的使用
### 纯函数： 
  纯函数是对给定的输入返还相同输出的函数，无副作用
...
### 高阶函数
  以函数作为输入或者输出的函数被称为高阶函数，通过高阶函数抽象过程，注重结果
  - 使用场景
    防抖函数
    组合compose
...
# 54. CORS请求的过程（Cross-Origin Resource Sharing）
1. 浏览器：发现为跨源请求，直接发出CORS请求，自动在头信息中，添加一个Origin字段（origin 参数的值指定了允许访问该资源的外域 URI）
```js
...
### 简单跨域请求的部分响应头如下：
- Access-Control-Allow-Origin（必含）- 不可省略，否则请求按失败处理。该项控制数据的可见范围，如果希望数据对任何人都可见，可以填写"*"。（origin 参数的值指定了允许访问该资源的外域 URI，如果服务端指定了具体的域名而非“*”，那么响应首部中的 Vary 字段的值必须包含 Origin）
```js
